...
public class Tester extends PApplet {
	...
	public static void main(String[] args) {
		PApplet.main("com.nikolamarincic.fpscaler.Tester");
	}

	public void settings() {
		size(1024, 768, P3D);
		pixelDensity(2); // high resolution
	...
	public void setup() {
		...
		for (int i = 0; i < COUNT; i++) {
			...
			/*
			 * // alternate color scheme float prob = random(100); if (prob < 30) {
			 * style[i*2] = colorBlended(random(1), 255,0,100, 255,0,0, 210); } else if
			 * (prob < 70) { style[i*2] = colorBlended(random(1), 0,153,255, 170,225,255,
			 * 210); } else if (prob < 90) { style[i*2] = colorBlended(random(1), 200,255,0,
			 * 150,255,0, 210); } else { style[i*2] = color(255,255,255, 220); }
			 */
	...
	public int colorBlended(float fract, float r, float g, float b, float r2, float g2, float b2, float a) {
